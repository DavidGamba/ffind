...
func main() {
	...
	fileTypeWithFileAndDir := opt.StringSliceMulti("t", 1, 1, "type")
	noFileType := opt.StringSliceMulti("T", 1, 1, "no-type")
	matchExtensionList := opt.StringSliceMulti("e", 1, 1, "extension")
	ignoreExtensionList := opt.StringSliceMulti("E", 1, 1, "no-extension")
